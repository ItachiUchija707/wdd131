...
function createRacesCard(racesArray) {
    const racesSection = document.querySelector(".grand-prix");
    ...
    racesArray.forEach(race => {
        ...
        `
            ...
            <p><span class="race-start">${race.dateStart}</span> - <span class="race-end">${race.dateEnd}</span></p>
            <figure>
                <img src="${race.imageUrl}" alt="${race.grandPrix} map" width="400" height="250" loading=lazy crossorigin="anonymous">
                <figcaption>${race.info}</figcaption>
            </figure>
        `
        racesSection.appendChild(raceCard);
    });
}
...
// filter races with an array to exceed expectations
...
{   
    ...
    createRacesCard(races);
    createChampionshipTable(drivers.sort((a,b) => b.points - a.points));
